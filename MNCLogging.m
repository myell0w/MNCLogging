...
const NSString * const kMNCLogInternalDontOutputMe = @"don't output me";

static BOOL MNCTypeCodeIsCharArray(const char *typeCode);
static NSString *MNCStringFromBoolOrCharValue(BOOL boolOrCharvalue);
static NSString *MNCStringFromFourCharCodeOrUnsignedInt32(FourCharCode fourcc);
static NSString *MNCStringFromNSDecimalWithCurrentLocal(NSDecimal dcm);
static NSString *MNCStringFromTypeAndValue(const char * typeCode, void * value);
...
NSString *_MNCLogToString(NSString *file, unsigned int line, ...)
{
    ...
    while (true) {
        ...
        if (*(void **)argument == (__bridge void *)kMNCLogInternalDontOutputMe) {
            break;
        }
        ...
        if ((strcmp(argumentTypeEncoding, @encode(id)) == 0) && (strncmp(argumentName, "@\"", 2) == 0)) {
            ...
            NSString *stringRepresentation = MNCStringFromTypeAndValue(argumentTypeEncoding,  argument);
...
NS_INLINE BOOL MNCTypeCodeIsCharArray(const char *typeCode)
{
...
NS_INLINE NSString* MNCStringFromBoolOrCharValue(BOOL boolOrCharvalue)
{
...
NS_INLINE NSString *MNCStringFromFourCharCodeOrUnsignedInt32(FourCharCode fourcc)
{
...
NS_INLINE NSString *MNCStringFromNSDecimalWithCurrentLocal(NSDecimal dcm)
{
    return NSDecimalString(&dcm, [NSLocale currentLocale]);
}

NS_INLINE NSString * MNCStringFromTypeAndValue(const char * typeCode, void * value)
{
...
return (func)(*(typeToMatch*)value)
    ...
    IF_TYPE_MATCHES_INTERPRET_WITH(BOOL,MNCStringFromBoolOrCharValue);
    IF_TYPE_MATCHES_INTERPRET_WITH(NSDecimal,MNCStringFromNSDecimalWithCurrentLocal);
...
return [NSString stringWithFormat:(formatString), (*(typeToMatch*)value)]
    ...
    IF_TYPE_MATCHES_INTERPRET_WITH(FourCharCode, MNCStringFromFourCharCodeOrUnsignedInt32);
    ...
    if(MNCTypeCodeIsCharArray(typeCode))
